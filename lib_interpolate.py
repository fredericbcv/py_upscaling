...
################################
def linear_interpolation(x,y, x_,y_, img):
    ...
    # Edge management
    if x_int < img.size[0]-1:
        x_int_add_1 = x_int+1
    else:
        x_int = img.size[0]-1
        x_int_add_1 = img.size[0]-1
    if y_int < img.size[1]-1:
        y_int_add_1 = y_int+1
    else:
        y_int = img.size[1]-1
        y_int_add_1 = y_int
    ...
    if x_frac == 0:
        # y interpolation
        y1 = img.load()[x_int, y_int]
        y2 = img.load()[x_int, y_int_add_1] 
        ...
        # x interpolation
        y1 = img.load()[x_int,  y_int] 
        y2 = img.load()[x_int_add_1,y_int] 
        y1 = tuple(map(lambda value: value * (1-x_frac), y1))
        y2 = tuple(map(lambda value: value * x_frac,     y2))

    return tuple_int(tuple_add(y1,y2))


def bilinear_interpolation(x,y, x_,y_, img):
    ...
    if x_frac == 0 or y_frac == 0:
        return linear_interpolation(x,y,x_,y_,img)

    else:
        f11 = img.load()[x_int,y_int]
        f12 = img.load()[x_int,y_int_add_1]
        f21 = img.load()[x_int_add_1,y_int]
        f22 = img.load()[x_int_add_1,y_int_add_1]
